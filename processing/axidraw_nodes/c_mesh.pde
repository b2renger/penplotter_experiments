...
class Mesh {
  ...
  void draw_mesh(float disto ) {
    meshDistortion = disto;
    pushMatrix();
    translate(ex, wy, ze);
    int iuMax, ivMax;
    ...
    if (pitchtracking) {
      minH = map(pitch, 24, 72, 360, 0);
      maxH = minH + 50;
      minS =constrain( map(pitch, 24, 48, 0, 100), 0, 100);
      maxS = minS+25;
    }
    strokeWeight(0.1);
    for (int iv = 0; iv <= ivMax; iv++) {
      if (drawMode == TRIANGLES) {
        ...
        for (int iu = 0; iu <= iuMax; iu++) {
          ...
          PVector p1 = new PVector(points[iv][iu].x, points[iv][iu].y, points[iv][iu].z);      
          PVector p2 = new PVector(points[iv+1][iu+1].x, points[iv+1][iu+1].y, points[iv+1][iu+1].z);         
          PVector p3 = new PVector(points[iv+1][iu].x, points[iv+1][iu].y, points[iv+1][iu].z);
          PVector s1 = p2.sub(p1);
          PVector s2 = p3.sub(p1);
          PVector dir = s1.cross(s2);
          dir.normalize();

          float r1 = meshDistortion * dir.x;
          float r2 = meshDistortion * dir.y;
          float r3 = meshDistortion * dir.z;
         ...
         // endShape();
          ...
          p2 = new PVector(points[iv][iu].x, points[iv][iu].y, points[iv][iu].z);      
          p1 = new PVector(points[iv+1][iu+1].x, points[iv+1][iu+1].y, points[iv+1][iu+1].z);         
          p3 = new PVector(points[iv][iu+1].x, points[iv][iu+1].y, points[iv][iu+1].z);
          s1 = p2.sub(p1);
          s2 = p3.sub(p1);
          dir = s1.cross(s2);
          dir.normalize();

          r1 = meshDistortion * dir.x;
          r2 = meshDistortion * dir.y;
          r3 = meshDistortion * dir.z;
        ...
        for (int iu = 0; iu <= iuMax; iu++) {
          ...
          float randAmtMax = 0.1;
          float randAmtMin = -randAmtMax;
          float r1 = meshDistortion * random(-1, 1);
          float r2 = meshDistortion * random(-1, 1);
          float r3 = meshDistortion * random(-1, 1);
          float rand1 = random(randAmtMin, randAmtMax);
          float rand2 = random(randAmtMin, randAmtMax);
          float rand3 = random(randAmtMin, randAmtMax);

          vertex(points[iv][iu].x+r1+rand1, points[iv][iu].y+r2+rand2, points[iv][iu].z+r3+rand3);
          vertex(points[iv+1][iu].x+r1+rand1, points[iv+1][iu].y+r2+rand2, points[iv+1][iu].z+r3+rand3);
          vertex(points[iv+1][iu+1].x+r1+rand1, points[iv+1][iu+1].y+r2+rand2, points[iv+1][iu+1].z+r3+rand3);
          vertex(points[iv][iu+1].x+r1+rand1, points[iv][iu+1].y+r2+rand2, points[iv][iu+1].z+r3+rand3);
  ...
  void draw_points(float disto ) {
   ...
   // colorMode(HSB, 360, 100, 100, 100);
    ...
    if (pitchtracking) {
      minH = map(pitch, 0, 72, 360, 180);
      maxH = minH + 50;
      minS =constrain( map(pitch, 0, 48, 0, 100), 0, 100);
      maxS = minS+10;
    }
    ...
    for (int iv = 0; iv <= ivMax; iv++) {
      for (int iu = 0; iu <= iuMax; iu++) {
        ...
        strokeWeight(2);
        //stroke(random(minH, maxH), random(minS, maxS), random(minB, maxB), meshAlpha);
        beginShape(POINTS);
        /*
        float randAmtMax = 0.1;
         float randAmtMin = -randAmtMax;
        ...
        vertex(r1, r2, r3);
        //pg.vertex(points[iv][iu].x+r1+rand1, points[iv][iu].y+r2+rand2, points[iv][iu].z+r3+rand3);
        endShape();
      }
    }
    popStyle();
